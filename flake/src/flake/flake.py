...
class Flake():
    def __init__(self, id, actions):
        ...
        print("\n\n\n\n ------------------------Graph: ",id,"__FLAKE INIT -------------------------------")
        #print("\n Node Types: ", self.nodetypes)
        #print("\n Edge Types: ", self.edgetypes)
    ...
    def add_edge(self, type, src_node, dst_node, features, jiffies):
        #print("\n Comes Here to edge \n")
        id = len(self.edges)
        #print(" Total Length Flake Dict: ", len(self.nodes))
        #print(" Flake Dict \n", self.nodes.keys)
        '''for key,value in self.nodes.items():
          print("  Flake Key: ",key, " value: {}".format(value.getType()))
        print( " ID: ",id ,"\t node_src: ", src_node,"\t node_dst: ", dst_node)
        print("----------------------------------------------")'''
        self.edges[id] = edge.Edge(type, self.nodes[src_node], self.nodes[dst_node], jiffies, id)
        self.edgetypes[type].append(id)
